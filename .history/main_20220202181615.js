...
const allProject = document.querySelector('.project_container')
const project = document.querySelectorAll('.project')
const nextBtn = document.querySelector('.next_arrow')
const prevBtn = document.querySelector('.prev_arrow')
let currentX = 0
let currentIndex = 0
console.dir(allProject)
console.dir(project)
console.dir(nextBtn)
nextBtn.addEventListener('click', () => {
  if (project.length > currentIndex + 1) {
    currentX += project[currentIndex].clientWidth
    console.log(currentX)
    currentIndex += 1
    allProject.style.transform = `translateX(-${currentX}px)`
  } else initializeSlider()
})

prevBtn.addEventListener('click', () => {
  if (currentX === 0) {
    allProject.style.transform = `translateX(-${
      allProject.clientWidth - project[0].clientWidth
    }px)`
    currentIndex = project.length - 1
    console.log(currentX)
  } else if (project.length >= currentIndex + 1) {
    currentX -= project[currentIndex].clientWidth
